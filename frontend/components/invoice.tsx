...
export function Invoice({ order }: InvoiceProps) {
    ...
    return (
        <div className="min-h-screen bg-white">
            ...
            {/* Invoice Content - Fixed width with scaling */}
            <div className="invoice-wrapper">
                <div className="invoice-content">
                        ...
                        <table className="w-full border-collapse">
                            <thead>
                                <tr className="bg-[#1e3a8a] text-white">
                                    <th className="border border-gray-300 px-4 py-3 text-left text-sm">N°</th>
                                    <th className="border border-gray-300 px-4 py-3 text-left text-sm">Désignation</th>
                                    <th className="border border-gray-300 px-4 py-3 text-center text-sm">Qté</th>
                                    <th className="border border-gray-300 px-4 py-3 text-right text-sm">P.U</th>
                                    <th className="border border-gray-300 px-4 py-3 text-right text-sm">Total</th>
                                </tr>
                            </thead>
                            <tbody>
                                {order.order_items.map((item, index) => (
                                    <tr key={item.id}>
                                        ...
                                        <td className="border border-gray-300 px-4 py-3 text-sm">{item.product_name}</td>
                                        <td className="border border-gray-300 px-4 py-3 text-center text-sm">{item.quantity}</td>
                                        <td className="border border-gray-300 px-4 py-3 text-right text-sm whitespace-nowrap">
                                            {new Intl.NumberFormat("fr-GN").format(item.unit_price)} GNF
                                        </td>
                                        <td className="border border-gray-300 px-4 py-3 text-right text-sm font-semibold whitespace-nowrap">
                                            {new Intl.NumberFormat("fr-GN").format(item.total_price)} GNF
                                        </td>
                                    </tr>
                                ))}
                                <tr className="font-bold bg-gray-50">
                                    <td colSpan={4} className="border border-gray-300 px-4 py-3 text-left text-sm">Total</td>
                                    <td className="border border-gray-300 px-4 py-3 text-right text-base whitespace-nowrap">
                    ...
                    <div className="flex justify-between">
                        <div>
                            <p className="font-semibold text-base">Le Directeur Général</p>
                        </div>
                        <div className="text-right">
                            <p className="font-semibold text-base">Le client</p>
                        </div>
                    </div>
                </div>
            </div>
